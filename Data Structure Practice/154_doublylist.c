...
void dlist_remove(List *list, int element)
{
    if (dlist_isEmpty(list)) return;

    DListNode *current = list->_head;
...
void dlist_removeAt(List *list, int index)
{
    if (dlist_isEmpty(list) || index < 0 || index >= list->_size)
        return;
...
int main(int argc, char const *argv[])
{
    ...
    dlist_remove(&myList, 10);
    dlist_removeAt(&myList, 2);
